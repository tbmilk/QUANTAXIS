...
| 组件 | 2.0.x | 2.1.0 | 说明 |
|------|-------|-------|------|
| **Python** | 3.5-3.10 | **3.9-3.14** | ⚠️ 需升级 |
...
可能原因:
1. 未启用Rust组件 → 安装`pip install -e .[rust]`
2. Python版本过低 → 升级到3.11+（或3.14作为目标版本）
3. 依赖版本过低 → 升级pymongo, pandas等
4. Python 3.14下分析包未使用 `-reloaded` 版本 → 安装`pip install -e .[analysis]`自动处理
...
- [ ] Python版本 >= 3.9（目标: 3.14，正式环境: venv314）
...
**完成日期**: 2026-05-18
**验证状态**: ✅ 100%兼容性验证通过（Python 3.9-3.14）
**作者**: @yutiansut @quantaxis
