...
2. **opentdx**
   - 支持：股票、ETF、普通指数、期货、除权除息等主链路
   - 使用：`package='opentdx'` 或 `package='otdx'`
   - 安装：Python 3.12+ 可使用 `pip install 'quantaxis[opentdx]'`
   - 说明：普通指数历史 K 线保留 `up_count` / `down_count`；ETF 保持既有 QUANTAXIS schema
   - 额外入口：扩展市场列表、板块列表/成分、资金流、公司资料等 fetch-only 能力

#### opentdx 已验证能力矩阵

以下结果来自 2026-05-18 的真实网络验收：
...
| globalfuture | 1365 行 | 1500 行 | `None` | - | - | list/day 可用，min 不可用 |
| globalindex | 27986 行 | `None` | `None` | - | - | 仅 list 可用 |
| option | 66 行 | `None` | `None` | - | - | 仅 list 可用 |

上游当前不支持或没有可用返回的扩展市场能力：
...
这些对象的 `list/day/min` 在验收中均返回空表或 `None`，属于上游限制，不应按代码故障处理。
